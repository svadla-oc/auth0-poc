...
Activities.helpers({
  ...
  newList() {
    return Lists.findOne(this.newListId);
  },
  oldList() {
  ...
  oldCard() {
    return Cards.findOne(this.oldCardId);
  },
  comment() {
  ...
  newListLink() {
    return Blaze.toHTML(HTML.A({
      href: this.newList().absoluteUrl(),
      'class': 'action-list',
    }, this.newList().title));
  },
  oldListLink() {
    return Blaze.toHTML(HTML.A({
      href: this.oldList().absoluteUrl(),
      'class': 'action-list',
    }, this.oldList().title));
  },
  oldCardLink() {
    return Blaze.toHTML(HTML.A({
      href: this.oldCard().absoluteUrl(),
      'class': 'action-card',
    }, this.oldCard().title));
  },
  checklist() {
  ...
  descriptionListLink() {
    return Blaze.toHTML(HTML.A({
      href: this.list().absoluteUrl(),
      'class': 'action-list',
    }, this.list().description));
  },
  descriptionCardLink() {
    return Blaze.toHTML(HTML.A({
      href: this.card().absoluteUrl(),
      'class': 'action-list',
...
if (Meteor.isServer) {
  ...
  Meteor.startup(() => {
    ...
    Activities._collection._ensureIndex({ listId: 1, createdAt: -1 });
  });
