...
Boards.attachSchema(new SimpleSchema({
  ...
  'members': {
    ...
    autoValue() { // eslint-disable-line consistent-return
      if (this.isInsert && !this.isSet) {
        return [{
          userId: this.userId || "OpenClinica_user",
          isAdmin: true,
          isActive: true,
          group: "public",
        }];
...
Boards.helpers({
  ...
  activities() {
    let activities = Activities.find({ boardId: this._id }, { sort: { createdAt: -1 }}).fetch();
    let result = [];
    let formIds = new Object();
    ...
    _.each(activities, (activity, index) => {
      if (activity.cardId && (activity.activityType === "addVersionCard" || activity.activityType === "updateVersionCard" || activity.activityType === "archiveVersionCard" || activity.activityType === "restoreVersionCard" || activity.activityType === "renameCard" || activity.activityType === "updateDescriptionCard")) {
        let form = Cards.findOne(activity.cardId);
        if (!_.contains(formIds[activity.activityType], form.form_id)) {
          let value = [];
          if (index === 0) {
            value.push(form.form_id);
            formIds[activity.activityType] = value;
          } else {
            if (formIds[activity.activityType]) {
              value = formIds[activity.activityType];
            }
            value.push(form.form_id);
            formIds[activity.activityType] = value;
          }
  ...
  activeMembers() {
    ...
    let resultMember = _.filter(this.members, function(member) {
        return member.isActive === true && member.userId !== "OpenClinica_user";
    });

    // add user with same group
    if (Meteor.user()) {
      if (Meteor.user().group) {
        Meteor.subscribe('user-by-group', Meteor.user().group);
        let users = Users.find({ 'group': Meteor.user().group}).fetch();
        _.each(users, (user) => {
          let temporaryUser = {
            userId: user._id,
            isAdmin: true,
          ...
          if (!_.findWhere(resultMember, {userId: user._id})) {
            resultMember[resultMember.length] = temporaryUser;
          }
        });
      }
    }
  ...
  hasGroup(group) {
    return !!_.findWhere(this.members, {group: group, isActive: true});
  },
...
Boards.mutations({
  ...
  setGroup(memberId, group) {
    const memberIndex = this.memberIndex(memberId);
    return {
      $set: {
        [`members.${memberIndex}.group`]: group,
      },
    };
  },

  setParentId(_parentId) {
    return { $set: { _parentId }};
  }
});
    ...
    if (doc._parentId) {
      if (userId) {
        Activities.insert({
          ...
          boardId: doc._id,
          oldBoardId: doc._parentId,
        });
      }
      const allLists = Boards.findOne(doc._parentId).lists().fetch();
      _.each(allLists, (list) => {
        list._parentId = list._id;
        delete list._id;
        delete list.createdAt;
        delete list.modifiedAt;
        list.boardId = doc._id;
        Lists.insert(list);
      });
    } else {
      if (userId) {
