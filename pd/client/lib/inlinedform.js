...
InlinedForm = BlazeComponent.extendComponent({
  ...
  open(evt) {
    ...
    list = Lists.findOne(Session.get('currentList'));
    // disable open when archived
    if ((card && !card.archived) || (list && !list.archived) || (list === undefined && card === undefined)) {
      evt && evt.preventDefault();
      // Close any opened slideout
      if (evt.currentTarget.classList.contains('js-card-composer') || evt.currentTarget.classList.contains('js-list-composer')){
