...
BlazeComponent.extendComponent({
  ...
  inviteUser(idNameEmail, group) {
    ...
    if (group)
      defaultGroup = group;
    Meteor.call('inviteUserToBoard', idNameEmail, boardId, defaultGroup, (err, ret) => {
  ...
  events() {
    return [{
      ...
      'click .js-select-member'() {
        const userId = this.currentData()._id;
        Meteor.subscribe('user-extra-group', userId);
        const user = Users.findOne(userId);
        const currentBoard = Boards.findOne(Session.get('currentBoard'));
        if (!currentBoard.hasMember(userId)) {
          this.inviteUser(userId, user.group);
        }
      },
