...
Meteor.publish('boards', function() {
  ...
  const group = currentUser.group ? currentUser.group : '';
  const {starredBoards = []} = currentUser.profile;
  ...
  return Boards.find({
    ...
    $or: [
      ...
      { members: { $elemMatch: { isActive: true, group: group }}},
      { members: { $elemMatch: { isActive: true, group: 'public' }}},
    ],
  }, {
    fields: {
      _id: 1,
